...
```bash
npm install maplibre-gl-lidar-tidop
```
...
import { LidarControl } from "maplibre-gl-lidar-tidop";
import "maplibre-gl-lidar-tidop/style.css";
...
import { LidarControlReact, useLidarState } from "maplibre-gl-lidar-tidop/react";
import "maplibre-gl-lidar-tidop/style.css";
import "maplibre-gl/dist/maplibre-gl.css";
...
maplibre-gl-lidar-tidop supports [Entwine Point Tile (EPT)](https://entwine.io/en/latest/entwine-point-tile.html) datasets, a widely-used format for serving large point clouds over HTTP with viewport-based streaming.
...
maplibre-gl-lidar-tidop works with Next.js out of the box, including Turbopack (the default bundler in Next.js 15+). The library bundles browser-safe shims for Node.js modules that are statically referenced but never executed in browsers.
...
docker pull ghcr.io/opengeos/maplibre-gl-lidar-tidop:latest

# Run the container
docker run -p 8080:80 ghcr.io/opengeos/maplibre-gl-lidar-tidop:latest
```

Then open http://localhost:8080/maplibre-gl-lidar-tidop/ in your browser to view the examples.

### Build Locally

```bash
# Build the image
docker build -t maplibre-gl-lidar-tidop .

# Run the container
docker run -p 8080:80 maplibre-gl-lidar-tidop
```
